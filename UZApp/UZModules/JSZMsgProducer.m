...
+ (NSString *)privateMicVideoDownSenderName:(NSString *)senderName senderId:(NSString *)senderId{
    OrderedDictionary *dict = [OrderedDictionary dictionaryWithObjectsAndKeys:@"_mic_video_down",@"event",senderId,@"sender_ID",@"所有人",@"target_ID",senderName,@"sender_NAME",@"私聊",@"media_TYPE",@{@"message":@"申请视频下麦"},@"data", nil];
    return DICTTOSTR(dict);
}
...
+(NSString *)offerAgreePriMicOnFrom:(NSString *)senderId to:(NSString *)sendToId sdp:(RTCSessionDescription *)sdp {
    NSString *event = @"_offer_agree_video_private";
...
+ (NSString *)pubOfflineSenderId:(NSString *)senderId senderName:(NSString *)senderName onMic:(BOOL)mic closeManner:(NSString *)closeManner {
    NSString *media_type;
    if (mic) {
        media_type = @"公开";
    }else{
    ...
    OrderedDictionary *dict = [OrderedDictionary dictionaryWithObjectsAndKeys:@"_offline",@"event",senderId,@"sender_ID",@"server",@"target_ID",senderName,@"sender_NAME",media_type,@"media_TYPE",@{@"message":@"正常退出会议室"},@"data",closeManner,@"Close_Manner", nil];
    return DICTTOSTR(dict);
